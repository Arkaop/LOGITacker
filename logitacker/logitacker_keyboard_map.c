...
uint32_t logitacker_keyboard_map_wc_to_hid_reports(hid_keyboard_report_t **p_out_report_seq, uint32_t *out_rep_seq_len,
    ...
    } else if (in_layout == LANGUAGE_LAYOUT_OSXFR) {
        switch (in_rune) {
            LAYOUT_OSXFR(LAYOUT_SWITCH_CASE)
            default:
                return NRF_ERROR_INVALID_PARAM;
        }
    } else {
...
logitacker_keyboard_map_lang_t logitacker_keyboard_map_lang_from_str(char * lang_str) {
    ...
    if (strcmp(lang_str, "osxfr") == 0 || strcmp(lang_str, "OSXFR") == 0 ) return LANGUAGE_LAYOUT_OSXFR;

    lab_default:
    NRF_LOG_WARNING("unknown language layout '%s' ... using 'fr' as default", nrf_log_push(lang_str));
    return LANGUAGE_LAYOUT_FR; // default
}
